...
def paint(e):
    ...
    if shape != 7:
        cv.create_line(
            (lastx, lasty, x, y),
            width=tools[selectedindex].size,
            fill=tools[selectedindex].color,
            tags=tag,
        )
    lastx, lasty = x, y
...
def exit_function():
    if messagebox.askyesno(
        "Close the window", "Do you want to close the window ?", icon="warning"
    ):
        window.destroy()
    else:
        pass
